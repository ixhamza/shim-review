...
Subject: CN = iXsystems, Inc. O = iXsystems, Inc. L = Campbell S = California C = US SERIALNUMBER = 4763415 2.5.4.15 = Private Organization 1.3.6.1.4.1.311.60.2.1.2 = Delaware 1.3.6.1.4.1.311.60.2.1.3 = US
```

*******************************************************************************
### What product or service is this for?
*******************************************************************************
TrueNAS is a Debian trixie-based storage solution available in a free Community Edition and a supported Enterprise Edition. It offers ZFS-powered file sharing over SMB/CIFS and NFS, iSCSI block storage, snapshots, replication, encryption, high-availability clustering, proactive hardware monitoring, and enclosure management.
...
*******************************************************************************
We use Debian's downstream Secure Boot implementation. Our GRUB repository: https://github.com/truenas/grub2.
...
* February 2025
  ...
  * CVE-2025-1125
*******************************************************************************
Yes. Our GRUB is based on Debian's grub2 2.12-9 which includes all listed CVE fixes.
...
*******************************************************************************
We have no previously signed shim. Our [prior submission](https://github.com/rhboot/shim-review/issues/500) was closed before approval.
...
*******************************************************************************
Yes. Our kernel is based on 6.18 and includes all three commits: https://github.com/truenas/linux/.
...
*******************************************************************************
Yes. Our kernel is based on an upstream LTS release with custom patches for storage bug fixes, performance enhancements, and additional storage capabilities. We track upstream linux-stable and regularly incorporate security and stability fixes. Complete list of our patches: https://github.com/truenas/linux/compare/v6.18.13...truenas/linux-6.18.

*******************************************************************************
### Do you use an ephemeral key for signing kernel modules?
### If not, please describe how you ensure that one kernel build does not load modules built for another kernel.
*******************************************************************************
Yes, in-tree kernel modules are signed with an ephemeral per-build key. For out-of-tree modules (ZFS, SCST, and NVIDIA), we use our signing key stored in a FIPS-certified YubiHSM on our signing server. We enforce strict ABI compatibility via `CONFIG_MODVERSIONS`, which ensures the kernel only accepts modules built for its exact configuration and CRC signature.
...
*******************************************************************************
`c9d8e8fc96e97db6e9c3e49e58afb713ade197833a718fc0255456151b472f16`

*******************************************************************************
### How do you manage and protect the keys used in your shim?
Describe the security strategy that is used for key protection. This can range from using hardware tokens like HSMs or Smartcards, air-gapped vaults, physical safes to other good practices.
*******************************************************************************
Root CA and signing keys are stored in FIPS-certified YubiHSM 2 devices with restricted access. The signing key is generated inside the signing server HSM and used for all signing operations. The signing key never leaves the hardware.
...
```
    ...
    X509v3 Subject Key Identifier:
        65:AA:C2:5C:DD:7B:93:8E:FB:FC:2C:CC:E8:F0:77:AE:4C:AD:DF:A8
...
*******************************************************************************
```
all_video boot btrfs cat chain configfile cpuid cryptodisk echo efifwsetup efinet ext2 f2fs fat font gcry_arcfour gcry_blowfish gcry_camellia gcry_cast5 gcry_crc gcry_des gcry_dsa gcry_idea gcry_md4 gcry_md5 gcry_rfc2268 gcry_rijndael gcry_rmd160 gcry_rsa gcry_seed gcry_serpent gcry_sha1 gcry_sha256 gcry_sha512 gcry_tiger gcry_twofish gcry_whirlpool gettext gfxmenu gfxterm gfxterm_background gzio halt help hfsplus iso9660 jfs jpeg keystatus linux loadenv loopback ls lsefi lsefimmap lsefisystab lssal luks luks2 lvm mdraid09 mdraid1x memdisk minicmd normal ntfs part_apple part_gpt part_msdos password_pbkdf2 peimage play png probe raid5rec raid6rec reboot regexp search search_fs_file search_fs_uuid search_label serial sleep smbios squash4 test tpm true video xfs zfs zfscrypt zfsinfo
```
...
*******************************************************************************
Our GRUB2 originates from Debian Trixie, full version 2.12-9+truenas2. https://github.com/truenas/grub2.
...
*******************************************************************************
UEFI firmware verifies shim, shim verifies GRUB, GRUB verifies the kernel via shim's protocol, and the kernel enforces lockdown when Secure Boot is active.
...
*******************************************************************************
We haven't reviewed other applications yet but plan to contribute to the review process going forward.
